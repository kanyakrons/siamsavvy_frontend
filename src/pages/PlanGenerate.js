...
import { generatePlanByAi } from "../api/planApi";
...
const PlanGenerate = () => {
    ...
    const AiGenerate = async () => {
        ...
        try {
            const categoryLabels = selectedCategoriesCriteria.map(category => category.label).join(', ');

            const response = await generatePlanByAi(selectedDaysCriteria, categoryLabels, selectedProvincesCriteria.label);
        ...
        <div className="p-5 max-w-7xl mx-auto">
                    ...
                    <div className="mt-8">
                        ...
                        <p className="text-gray-400 text-sm">
                            {(() => {
                                ...
                                return `${formattedDate[0]} ${formattedDate[2].replace(",", "")} ${formattedDate[1]} ${formattedDate[3]}`;
                            })()}
                        </p>
                    </div>
                            ...
                            {isAiGenerate &&
                                <div className="w-full">
                                    <div className="flex flex-col text-left mb-3">
                                        <label className="text-sm font-semibold text-gray-600 mb-1">Days</label>
                                        <input
                                            type="number"
                                    ...
                                    <div className="flex flex-col text-left mb-3">
                                        ...
                                        <Select
                                            isMulti
                                            options={categoriesCriteria.map((c) => ({ value: c.id, label: c.name }))}
                                    ...
                                    <div className="flex flex-col text-left mb-3">
                                        <label className="text-sm font-semibold text-gray-600 mb-1">Province</label>
                                        <Select
                                            options={provincesCriteria.map((p) => ({ value: p, label: p }))}
                                    ...
                                    <div className="flex justify-center mt-8">
                                        <button onClick={AiGenerate} className="w-40 bg-purple-400 text-white rounded-xl py-2 hover:bg-purple-500 hover:shadow-lg transition duration-300 ease-in-out">
                                            Generate
                                        </button>
                                    </div>
                                </div>
                                    ...
                                    <div className="grid grid-cols-2 gap-4 mt-10">
                                        {filteredPlaces.length > 0 ? (
                                            filteredPlaces.map((place) => (
                                                <div key={place.id} className="w-full h-[250px] relative bg-white rounded-3xl shadow-lg aspect-square overflow-hidden transition-all duration-300 transform hover:scale-105 hover:shadow-2xl">
                                                    <Link to={`/places/${place.id}`}>
                                                        ...
                                                        <div className="h-[80px] bg-white px-5 py-2">
                                                            <p className="text-purple-400 font-semibold text-xs mb-1">{place.province}</p>
                                                            <p className="text-black font-semibold text-sm">{place.nameTh}</p>
                                                            <p className="text-black text-sm">{place.nameEn}</p>
                                                        </div>
                                                    </Link>
                                                </div>
                                            ))
                            ...
                            {planDetails && (
                                <div>
                                    ...
                                    <div className="flex overflow-x-auto mb-4">
                                        {planDetails.trip.itinerary.map((dayPlan) => (
                                            <button
                                                onClick={() => { setSelectedDay(parseInt(dayPlan.day) - 1) }}
                                                className={`px-4 py-2 mr-2 rounded-xl font-semibold focus:outline-none ${selectedDay === parseInt(dayPlan.day) - 1 ? 'bg-purple-400 text-white' : 'bg-gray-200 text-gray-800'
                                                    } transition duration-300 ease-in-out ${selectedDay === parseInt(dayPlan.day) - 1 ? 'hover:bg-purple-500' : 'hover:bg-gray-300'}`}
                                            >
                                                {`Day ${dayPlan.day}`}
                                            </button>
                                        ))}
                                    ...
                                    {planDetails.trip.itinerary[selectedDay]?.places ? (
                                        ...
                                        <div className="text-gray-500">No places available for this day</div>
                                    )}
                                </div>
                            )}
                        </div>
            ...
            {!isPlanning &&
                <div className="flex flex-col items-center mt-4">
                    ...
                    <button className={`px-4 py-2 mr-2 rounded-xl font-semibold text-white focus:outline-none bg-purple-400 transition duration-300 ease-in-out bg-purple-300 hover:bg-purple-500`}>Save</button>
                </div>
            }
        </div>
    );
